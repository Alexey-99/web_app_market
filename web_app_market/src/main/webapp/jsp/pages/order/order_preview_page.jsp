...
<title><fmt:message key="preview_order_page.title" /></title>
<!-- order = AttributeName.ATTRIBUTE_ORDER -->
<!-- is_have_products_pets = AttributeName.ATTRIBUTE_IS_HAVE_ORDER_PRODUCTS_PETS -->
<!-- is_have_products_feed_and_other = AttributeName.ATTRIBUTE_IS_HAVE_ORDER_PRODUCTS_FEED_AND_OTHER -->
					...
					<div class="col-9 products_order">
						<div class="products_order_top">
							<div class="text-center">
								<h3 class="text-uppercase">
									<fmt:message key="preview_order_page.title" />
								</h3>
							</div>
									...
									<div class="products_order_top_form h-100">
										<form class="h-100" style="width: 50vh"
											...
											<button style="min-width: 42vh"
												...
												<h5
													class="h-100 d-flex justify-content-center align-items-center ">
													<fmt:message key="preview_order_page.btn.ok" />
												</h5>
											</button>
										</form>
									</div>
						...
						<div
							...
							<c:forEach items="${order.getProductsPets()}" var="pet"
								varStatus="status">
								<c:if test="${is_have_products_pets.get(status.getIndex())}">
									<div class="col card_product">
										<div class="card h-100 card_product_inner"
											style="border: 2px var(--color1) solid;">
...
<path
												...
												<div class="discription">
													...
													<ul class="discription_botton">
														...
														<li><fmt:message key="basket_page.product.discount" />
															...
															alt="percent" /></li>
														<li><fmt:message
																key="basket_page.product.total_price" />
															${String.format("%,.2f", pet.getTotalPrice())}</li>
													</ul>
												</div>
												<div class="row body_btns w-100">
													<div class="col-12 body_btn">
														<div class="w-100 h-100 body_btn_input text-center"
															style="border: 2px solid var(--color1); transition: none; font-size: 20px;">
															<fmt:message
																key="preview_order_page.product.mmessage.have_market" />
														</div>
													</div>
												</div>
											</div>
								...
								<c:if test="${!is_have_products_pets.get(status.getIndex())}">
									<div class="col card_product">
										<div class="card h-100 card_product_inner"
											style="border: 2px var(--color4) solid;">
											<div class="card-img-top card_img"
												style="border: 1px solid var(--bs-card-border-color)">
												<c:if test="${pet.getImagePath() != null}">
													<img class="w-100 h-100" alt=""
...
<path
											...
											<div class="card-body card_body">
												...
												<div class="row body_btns w-100">
													<div class="col-12 body_btn">
														<div class="w-100 h-100 body_btn_input text-center"
															style="border: 2px solid var(--color4); transition: none; font-size: 20px; background: #ffadb4;">
															<fmt:message
																key="preview_order_page.product.mmessage.not_have_market" />
														</div>
													</div>
												</div>
											</div>
							...
							<c:forEach items="${order.getOtherProducts()}" var="item"
								varStatus="status">
								<c:if
									test="${is_have_products_feed_and_other.get(status.getIndex())}">
									<div class="col card_product">
										<div class="card h-100 card_product_inner"
											style="border: 2px var(--color1) solid;">
											<div class="card-img-top card_img"
...
<path
						...
						<div
							class="col-12 d-flex justify-content-around align-items-center mt-3 mb-3">
							<div class="products_order_top_form h-100">
								<form class="h-100" style="width: 50vh"
									action="${ServletName.MAIN_SERVLET_CONTROLLER_NAME}"
									method="get">
