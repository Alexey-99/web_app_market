...
public class ProductPetPaginationTag extends TagSupport {
	...
	private void printProducts(List<Entry<Pet, Long>> listProductPets, String locale, User user) throws IOException {
		...
		for (int i = indexFirstElement; i < listProductPets.size()
			...
			if (listProductPets.get(i).getValue() > 0) {
				...
				if (pet.getImagePath() != null) {
					builder.append("""
							...
							.append("=").append(pet.getImagePath()).append("""
									" />
									""");
				} else {
					builder.append(
				...
				if (locale.equalsIgnoreCase(RUSSIAN)) {
					...
					builder.append("<li> тип питомца: ").append(pet.getSpecie()).append("</li>");
					builder.append("<li> порода: ").append(pet.getBreed()).append("</li>");
					builder.append("<li> дата рождения: ").append(pet.getBirthDate().toString()).append("</li>");
				} else if (locale.equalsIgnoreCase(ENGLISH)) {
					builder.append("<li> number of product: p-").append(pet.getId()).append("</li>");
					builder.append("<li> specie: ").append(pet.getSpecie()).append("</li>");
					builder.append("<li> breed: ").append(pet.getBreed()).append("</li>");
					builder.append("<li> date birthday: ").append(pet.getBirthDate().toString()).append("</li>");
				} else {
					builder.append("<li> number of product: p-").append(pet.getId()).append("</li>");
					builder.append("<li> specie: ").append(pet.getSpecie()).append("</li>");
					builder.append("<li> breed: ").append(pet.getBreed()).append("</li>");
				...
				if (user != null && user.getRole().getId() >= UserRole.USER.getId() && user.isVerificatedEmail()) {
					builder.append("""
							...
							.append(createInputHidden(PARAMETER_COMMAND, COMMAND_ADD_PRODUCT_PETS_TO_ORDER))
				...
				builder.append("""
						...
						""");
				if (locale.equalsIgnoreCase(RUSSIAN)) {
					builder.append("Последнее обновление ").append(pet.getUpdateDateTime().toStringRus())
							.append(" назад");
				} else if (locale.equalsIgnoreCase(ENGLISH)) {
					builder.append("Last update of the product ").append(pet.getUpdateDateTime().toStringEn())
							.append(" ago");
				} else {
					builder.append("Last update of the product ").append(pet.getUpdateDateTime().toStringEn())
							.append(" ago");
