...
import static by.koroza.zoo_market.web.command.name.command.CommandName.COMMAND_ADD_PRODUCT_FEEDS_AND_OTHER_TO_ORDER;
...
/**
 * The Class ProductFeedsAndOtherPaginationTag.
 */
public class ProductFeedsAndOtherPaginationTag extends TagSupport {

	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = 6804752093165421526L;
	...
	public int doStartTag() throws JspException {
		try {
			...
			setNumberLastPage(listProductFeedsAndOther);
			setNumberPage();
	...
	private void setNumberLastPage(List<Entry<FeedAndOther, Long>> listProduct) {
		int countPage = (int) Math.ceil((double) listProduct.size() / this.maxCountProductsOnPage);
		this.numberLastPage = countPage;
	}
	 ...
	 *
	 * @param listProductFeedsAndOther the list product feeds and other
	 * @param locale                   the locale
	 * @param user                     the user
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private void printProducts(List<Entry<FeedAndOther, Long>> listProductFeedsAndOther, String locale, User user)
			throws IOException {
		...
		for (int i = indexFirstElement; i < listProductFeedsAndOther.size()
			...
			if (listProductFeedsAndOther.get(i).getValue() > 0) {
				FeedAndOther feedAndOther = listProductFeedsAndOther.get(i).getKey();
				builder.append(
						"""
				...
				if (feedAndOther.getImagePath() != null) {
					builder.append("""
							...
							""").append(SERVLET_SHOW_IMAGE_NAME).append("?").append(PARAMETER_IMAGE_FILE_PATH)
							.append("=").append(feedAndOther.getImagePath()).append("""
									" />
									""");
				} else {
					builder.append(
				...
				if (locale.equalsIgnoreCase(RUSSIAN)) {
					...
					builder.append("<li> товар подходит для след. типов питомцев: ").append(feedAndOther.getPetTypes()
							.toString().substring(1, feedAndOther.getPetTypes().toString().length() - 1))
							.append("</li>");
					...
					builder.append("<li> description of product: ").append(feedAndOther.getDescriptions())
							.append("</li>");
					builder.append("<li> this product is suitable for the following types of pets: ")
							.append(feedAndOther.getPetTypes().toString().substring(1,
									feedAndOther.getPetTypes().toString().length() - 1))
							.append("</li>");
				} else {
					builder.append("<li> number of product: o-").append(feedAndOther.getId()).append("</li>");
					builder.append("<li> type of product: ").append(feedAndOther.getProductType()).append("</li>");
				...
				builder.append("""
						...
						""");
				if (locale.equalsIgnoreCase(RUSSIAN)) {
					builder.append("<li> цена: ").append(numberFormatterPrice.format(feedAndOther.getPrice()))
							.append("</li>");
					builder.append("<li> скидка: ").append(numberFormatterPrice.format(feedAndOther.getDiscount()))
							.append("""
									...
									<li> стоимость со скодкой:
											""")
							.append(" ").append(numberFormatterPrice.format(feedAndOther.getTotalPrice()))
							.append("</li>");
				} else if (locale.equalsIgnoreCase(ENGLISH)) {
					builder.append("<li> price: ").append(numberFormatterPrice.format(feedAndOther.getPrice()))
							.append("</li>");
					builder.append("<li> discount: ").append(numberFormatterPrice.format(feedAndOther.getDiscount()))
							.append("""
									<svg class="discription_botton_discont_procent_img" xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48">
										<path d="M289.899 516Q236 516 198 477.899t-38-92Q160 332 198.101 294t92-38Q344 256 382 294.101t38 92Q420 440 381.899 478t-92 38Zm-.017-60Q319 456 339.5 435.618q20.5-20.383 20.5-49.5Q360 357 339.618 336.5q-20.383-20.5-49.5-20.5Q261 316 240.5 336.382q-20.5 20.383-20.5 49.5Q220 415 240.382 435.5q20.383 20.5 49.5 20.5Zm380.017 440Q616 896 578 857.899t-38-92Q540 712 578.101 674t92-38Q724 636 762 674.101t38 92Q800 820 761.899 858t-92 38Zm-.017-60Q699 836 719.5 815.618q20.5-20.383 20.5-49.5Q740 737 719.618 716.5q-20.383-20.5-49.5-20.5Q641 696 620.5 716.382q-20.5 20.383-20.5 49.5Q600 795 620.382 815.5q20.383 20.5 49.5 20.5ZM202 896l-42-42 598-598 42 42-598 598Z"/>
									</svg>
				...
				builder.append("""
						...
						""");
				if (locale.equalsIgnoreCase(RUSSIAN)) {
					builder.append("количество единиц: ").append(listProductFeedsAndOther.get(i).getValue());
				} else if (locale.equalsIgnoreCase(ENGLISH)) {
				...
				if (user != null && user.getRole().getId() >= UserRole.USER.getId() && user.isVerificatedEmail()) {
					builder.append("""
							...
							" method="get">
									...
									value=" """).append(feedAndOther.getId()).append("\"/>")
							.append(createInputHidden(PARAMETER_COMMAND, COMMAND_ADD_PRODUCT_FEEDS_AND_OTHER_TO_ORDER))
							.append("\n").append(createInputHidden(PARAMETER_PRODUCT_ID, feedAndOther.getId()))
	 ...
	 *
	 * @param listProductfeedsAndOther the list productfeeds and other
	 * @param locale                   the locale
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private void printPagination(List<Entry<FeedAndOther, Long>> listProductfeedsAndOther, String locale)
			throws IOException {
		StringBuilder builder = new StringBuilder();
