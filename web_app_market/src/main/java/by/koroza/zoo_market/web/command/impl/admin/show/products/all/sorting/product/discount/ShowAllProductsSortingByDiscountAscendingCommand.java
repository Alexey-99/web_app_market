...
import by.koroza.zoo_market.service.sorting.product.impl.comparator.map.impl.product.discount.SortProductsByDiscountAscendingComparatorImpl;
...
public class ShowAllProductsSortingByDiscountAscendingCommand implements Command {
		...
		try {
			if (user != null && user.getRole().getId() == ADMIN.getId() && user.isVerificatedEmail()) {
				...
				session.setAttribute(ATTRIBUTE_MAP_PRODUCTS_AND_NUMBER_OF_UNITS_PRODUCT, SortingProductsImpl
						.getInstance().sortProductsMap(products, new SortProductsByDiscountAscendingComparatorImpl()));
