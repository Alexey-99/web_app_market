...
import by.koroza.zoo_market.service.sorting.comparator.map.impl.product.pet.birthdate.SortProductsByProductPetBirthDateAscendingComparatorImpl;
...
public class ShowAllProductsOffFilterSortingByProductPetBirthDateAscendingCommand implements Command {
		...
		try {
			if (user != null && user.getRole().getIdRole() == ADMIN.getIdRole() && user.isVerificatedEmail()) {
				...
				session.setAttribute(ATTRIBUTE_MAP_PRODUCTS_AND_NUMBER_OF_UNITS_PRODUCT, SortingProductsImpl.getInstance()
						.sortProductsMap(products, new SortProductsByProductPetBirthDateAscendingComparatorImpl()));
