package by.koroza.zoo_market.web.command.impl.admin.show.products.all.sorting.product.pet.type;
...
import by.koroza.zoo_market.service.sorting.product.impl.comparator.map.impl.product.pet.type.SortProductsByProductPetTypeDescendingComparatorImpl;
...
public class ShowAllProductsSortingByProductPetTypeDescendingCommand implements Command {
			...
			if (user != null && user.getRole().getId() == ADMIN.getId() && user.isVerificatedEmail()) {
				...
				session.setAttribute(ATTRIBUTE_MAP_PRODUCTS_AND_NUMBER_OF_UNITS_PRODUCT,
						SortingProductsImpl.getInstance().sortProductsMap(products,
								new SortProductsByProductPetTypeDescendingComparatorImpl()));
