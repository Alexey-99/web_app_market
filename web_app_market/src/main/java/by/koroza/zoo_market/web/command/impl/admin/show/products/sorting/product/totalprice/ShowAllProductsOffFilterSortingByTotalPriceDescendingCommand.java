package by.koroza.zoo_market.web.command.impl.admin.show.products.sorting.product.totalprice;
...
import by.koroza.zoo_market.service.sorting.comparator.map.impl.product.totalprice.SortProductsByTotalPriceDescendingComparatorImpl;
...
public class ShowAllProductsOffFilterSortingByTotalPriceDescendingCommand implements Command {
		...
		try {
			if (user != null && user.getRole().getIdRole() == ADMIN.getIdRole() && user.isVerificatedEmail()) {
				...
				session.setAttribute(ATTRIBUTE_MAP_PRODUCTS_AND_NUMBER_OF_UNITS_PRODUCT, SortingProductsImpl.getInstance()
						.sortProductsMap(products, new SortProductsByTotalPriceDescendingComparatorImpl()));
