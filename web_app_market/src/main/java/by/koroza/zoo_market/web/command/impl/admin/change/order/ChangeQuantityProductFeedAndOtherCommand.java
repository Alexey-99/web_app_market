...
import static by.koroza.zoo_market.web.command.name.command.CommandName.COMMAND_ADMIN_PAGE_SHOW_MORE_DETAILS_ABOUT_PRODUCT_FEEDS_AND_OTHER;
...
/**
 * The Class ChangeQuantityProductFeedAndOtherCommand.
 */
public class ChangeQuantityProductFeedAndOtherCommand implements Command {
			...
			if (user != null && user.isVerificatedEmail() && user.getRole().getId() == ADMIN.getId()) {
				...
				if (productId != null && productId.matches("^(\\d)+$")) {
					...
					router = new Router(new StringBuilder().append("/").append(MAIN_SERVLET_CONTROLLER_NAME).append("?")
							...
							.append(COMMAND_ADMIN_PAGE_SHOW_MORE_DETAILS_ABOUT_PRODUCT_FEEDS_AND_OTHER).append("&")
