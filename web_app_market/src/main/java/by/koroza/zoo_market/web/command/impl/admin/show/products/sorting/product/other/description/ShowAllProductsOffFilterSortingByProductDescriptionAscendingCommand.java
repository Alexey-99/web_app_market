...
import by.koroza.zoo_market.service.sorting.comparator.map.impl.product.other.description.SortProductsByProductDescriptionAscendingComparatorImpl;
...
public class ShowAllProductsOffFilterSortingByProductDescriptionAscendingCommand implements Command {
		...
		try {
			if (user != null && user.getRole().getIdRole() == ADMIN.getIdRole() && user.isVerificatedEmail()) {
				...
				session.setAttribute(ATTRIBUTE_MAP_PRODUCTS_AND_NUMBER_OF_UNITS_PRODUCT, SortingProductsImpl.getInstance()
						.sortProductsMap(products, new SortProductsByProductDescriptionAscendingComparatorImpl()));
