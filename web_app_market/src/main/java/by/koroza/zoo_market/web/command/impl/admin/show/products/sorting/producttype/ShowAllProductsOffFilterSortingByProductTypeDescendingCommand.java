...
import by.koroza.zoo_market.service.sorting.impl.id.SortProductsByIdDescendingImpl;
		...
		try {
			if (user != null && user.getRole().getIdRole() == UserRole.ADMIN.getIdRole()
				...
				session.setAttribute(ATTRIBUTE_MAP_PRODUCTS_AND_NUMBER_OF_UNITS_PRODUCT, SortingMapProducts
						.getInstance().sortProductsMap(products, new SortProductsByIdDescendingImpl()));
